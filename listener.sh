...
if [[ ! $mac =~ ^[0-9A-F]{2}:[0-9A-F]{2}:[0-9A-F]{2}:[0-9A-F]{2}$ ]]; then
...
while read -r line; do
    if [[ $line =~ ^Notification\ handle\ =\ 0x0036\ value:\ [0-9a-f]{2}\ [0-9a-f]{2}\ [0-9a-f]{2}\ [0-9a-f]{2}\ [0-9a-f]{2}$ ]]; then
        ...
        if [[ $temp =~ ^[0-9]{2}\.[0-9]{2}$ && $hum =~ ^[0-9]{2}$ && $batt =~ ^[0-9]{1,2}.[0-9]{1,2}$ ]]; then
            echo "${temp:0:2}.${temp:2:2},${hum},${batt}"
        else 
            echo "ERROR: unexpected input: ${line}"
            exit 1
        fi
    fi
